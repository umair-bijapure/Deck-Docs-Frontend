...
// const ProjectProgress: React.FC<ProjectProgressProps> = ({ projectId, progress: initialProgress }) => {
//   const [progress, setProgress] = useState<Progress[]>(initialProgress);
//   const [newTaskTitles, setNewTaskTitles] = useState<string[]>(Array(progress.length).fill(''));
...
//   const [selectedListIndex, setSelectedListIndex] = useState<number>(-1); // Initialize with -1 to indicate no list selected
...
// console.warn(progress,tasks,"GBGBGBGFFVFVFffffffffffffffffffffffffffffffffffffffffffVFVFVFFVVFTTCGTVHVVGTVGV")  
...
//   // Set the selected list index when the input is clicked
//   handleListSelect(headingIndex);
// };
//   return (
//     <div style={{ display: 'flex', gap: '20px' }}>
//       <DragDropContext onDragEnd={onDragEnd}>
//       {progress.map((heading, headingIndex) => (
//       <div key={heading._id} style={{ minWidth: '250px' }}>
//         <h3>{heading.title}</h3>
...
//               // Render tasks for the current list
// {heading.tasks.map((taskId, taskIndex) => {
//     // Find the task directly in the list
//     const task = tasks.find(task => task._id === String(taskId)); 
...
//         />
//         <button
//           onClick={() => addTaskToList(headingIndex, heading._id)} // Pass listId here
...
const ProjectProgress: React.FC<ProjectProgressProps> = ({ projectId, progress: initialProgress,team,users }) => {
  ...
  const onUpdate = async (updatedTask: Task) => {
    try {
      ...
      // For example:
      console.warn(updatedTask,"ininininninininininiinininninininininininininiinni")
...
const updateTask = async (updatedTask: Task, listId: string) => {
  try {
      ...
      console.warn(updatedTask,updatedTask._id,"777777777777777777777777777777777777777777777777777777777777777777777777777777777777777777777777777777")
