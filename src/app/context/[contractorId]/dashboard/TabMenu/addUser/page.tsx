...
const AddEmployee = (props: any): JSX.Element => {

  const {contractorId}= props as AddEmployeeProps
  const { onClick } = props as AddEmployeeProps
  const { onEmployeeAdded } = props as AddEmployeeProps
  ...
  const formSubmitHandler = async (e: any) => {
    ...
    try {
      const createOrganisation = await standardFetch({
        url: `${process.env.NEXT_PUBLIC_API_URL}/auth/${contractorId}/user`, // Replace parentOrganisationId with actual ID
        method: 'POST',
